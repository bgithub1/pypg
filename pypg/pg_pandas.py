...
#         return str(dt).split(' ')[0]
# 
# def get_sql(sql_string,engine=None,db_csv=None):
#     e = engine
#     if e is None:
# #         e = get_engine_from_csv("./db.csv")
#         e = get_engine_from_csv(db_csv if db_csv is not None else DB_CSV_DEFAULT_PATH)
...
#     
# 
# def put_df(df,table_name,engine=None,ifexists='append',save_index_of_df=True,schema=None,db_csv=None):
#     e = engine
#     if e is None:
# #         e = get_engine_from_csv("./db.csv")
#         e = get_engine_from_csv(db_csv if db_csv is not None else DB_CSV_DEFAULT_PATH)
...
#         
# def put_df_with_type_dict(df,table_name,type_dict,engine=None,ifexists='append',save_index_of_df=True,db_csv=None):
#     e = engine
#     if e is None:
# #         e = get_engine_from_csv("./db.csv")
#         e = get_engine_from_csv(db_csv if db_csv is not None else DB_CSV_DEFAULT_PATH)
...
# 
# 
# def put_df_fast(df,table_name_with_schema,eng):
#     col_names = df.columns.values
#     unnest_list = []
#     for c in col_names:
#         unnest_list.append("unnest(array%s)" %(list(df[c])))
...
#            SELECT %(UNNEST_LIST)s
#     ''' %{'TABLE_NAME':table_name_with_schema,'COL_NAMES':col_name_string,'UNNEST_LIST':all_unnests}
#     exec_sql_raw(sql, engine=eng)
#     pass
# 
# # def csv_to_db_copy(df,tableName,db_csv_path='./db.csv'):
# def csv_to_db_copy(df,tableName,db_csv_path=DB_CSV_DEFAULT_PATH):
#     '''
...
#     return
# 
# def df_to_db_fast(df,table_name,db_csv_path=None,sep_to_use=None,quoting_to_use=None):
#     sep = sep_to_use
#     if sep is None:
#         sep = ','
#     p = db_csv_path
#     if p is None:
#         p = DB_CSV_DEFAULT_PATH #"./db.csv"
#     psyconn = get_ps_cursor_from_csv(p)
#     f = sio.StringIO()
#     df.to_csv(f, index=False,header=False,sep=sep,quoting=quoting_to_use)
...
# def csv_to_db_fast(tableName,csv_path,db_csv_path=DB_CSV_DEFAULT_PATH):
#     '''
#         Copy a csv file to a postgres table using the raw sql copy command
#         and psycopg2
#         RIGHT NOW THE TABLE NAME MUST BELONG TO THE SCHEMA THAT IS 
...
#               ifexists_action='append',engine=None,db_csv=None):
#     '''
...
#     dbcsv = db_csv_path
#     if dbcsv is None:
#         dbcsv = DB_CSV_DEFAULT_PATH #"./db.csv"
...
#     psyconn.close()
# 
# def exec_sql_raw(sql,db_csv_path=None,engine=None):
#     '''
#         Called postgres sql statement when the sql references functions that are
#         not in the same schema as the login referenced by db_csv_path
#     '''
